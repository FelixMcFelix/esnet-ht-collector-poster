...
\begin{columns}
	...
	\block[bodywidthscale=0.999]{Architecture}{
		...
		?? Future network---\textbf{ESnet6}.
		
		?? Massive WAN. multi-\SI{100}{\giga\bit\per\second} at edge, to peers.
		
		?? Install something like Fig. \ref{fig:ht-arch} at every edge node, selectively mirror traffic.
		
		?? P4 \cite{DBLP:journals/ccr/BosshartDGIMRSTVVW14}
		
		?? Why go with this model? Data reduction and timestamping at line-rate. Control plane lets us control where filtering applies, and which actions happen (which collectors). Custom deployable firmware lets us evolve our collectors, formats, applications, and \textbf{adapt}.

		?? What actual hardware? Agilio CXs
		
		?? Describe how collectors make data available?
	}
\block[bodywidthscale=0.9999]{Point Rates}{
	\fontsize{32}{38.4}\selectfont
	Being able to accurately time every packet...
	\begin{tikzfigure}[Point rates. Note that $p_1$ and $p_2$ may be from different flows.\label{fig:pr}]
		\resizebox{\linewidth}{!}{\fontsize{10}{12}\selectfont
			\begin{tikzpicture}
...
%	\begin{wrapfigure}[6]{r}{.3\linewidth}
		...
		We've implemented a collector application to perform TCP flow analysis from High Touch packets, including the following algorithms:
		\begin{itemize}
		...
		We use a pipelined design (Fig. \ref{fig:collector-arch}) to maximise throughput where possible.
	...
	The ingress pipeline then has a peak capacity of \textbf{\SI{55.8}{\giga\bit\per\second} between all flows}, and \textbf{\SIrange{22.3}{34.9}{\giga\bit\per\second} per-flow} (depending on which analyses are performed).
...
\block[bodywidthscale=0.9999]{Measuring At Midpoints}{
	\fontsize{32}{38.4}\selectfont
	\begin{tikzfigure}[SRTT computation of unidirectional flows as seen by collectors. Bytes carrying data are marked by a red arrow.\label{fig:srtt}]
		\resizebox{0.8\linewidth}{!}{\fontsize{8}{9.6}\selectfont
				...
				\draw[<->] ($(e1circ.south) - (0.0,0.1)$) -- node[below]{$SRTT_{E_1}$} ($(bcirc.south) - (0.0,0.1)$);
				\draw[<->] ($(e2circ.south) - (0.0,0.3)$) -- node[below]{$SRTT_{E_2}$} ($(bcirc.south) - (0.0,0.3)$);
	...
	?? Same observations probnably apply to our BIF too.
}
	\column{0.33}
	...
	\block[bodywidthscale=0.9999]{TCP Flavour Identification}{
		\begin{tikzfigure}[Accuracy in telling apart TCP Cubic and BBR flows.\label{fig:lstm}]
			\adjincludegraphics[width=0.8\linewidth]{cubic-vs-bbr-accuracy-poster}
		\end{tikzfigure}
	...
	\block[bodywidthscale=0.9999]{Future Work}{
		\fontsize{32}{38.4}\selectfont
		\begin{itemize}
			\item Correlating results between collectors (and switch-to-switch time synchronisation \cite{DBLP:conf/sosr/KannanJC19}).
			\item Move control of which analyses are performed to the control plane.
			\item Investigate analysis \emph{on} SmartNICs.
		\end{itemize}
	}
